...
入札型直接対決。3回のオークションで最高入札者が「封筒」を獲得。封筒の中身（効果）は開封まで不明。
...
### Game 3の進行（新仕様：シンプルオークション）
1. 管理画面でフェーズを「Game 3」に切り替え
2. **第1ラウンド**：各チームが入札額を入力（現在の最高額より高い必要あり）
3. 管理画面で「✅ 入札確定」→ 最高入札者のARから入札額が減額され、封筒を獲得
4. 管理画面で「🔄 入札リセット」→ 次のラウンド準備
5. **第2・第3ラウンド**も同様に実施（合計3回）
6. **封筒開示**：3人の落札者がリアルの会場で封筒を一斉に開封し、効果を発表
7. **効果の手動適用**：管理画面のチーム管理テーブルで、各チームのAR残高をクリックして直接編集
   - 例：「+200万AR」のカード → AR残高に2,000,000を加算して保存
   - 例：「チームAとBのAR交換」→ それぞれの残高を入れ替えて保存
...
- `POST /api/game/game3-confirm` - Game 3の入札確定（ARを減額）
...
- `POST /api/team/update-balance` - チームのAR残高を直接更新（手動調整用）
